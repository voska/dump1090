...
var planeObject = {
	...
	funcUpdateLabel : function() {
		// This has to be HTML for now...
		label = "" + this.flight + "<br />";
		...
		if ((data.validposition == 1) && (this.reapable == false)) {
			...
			if ((changeLat == true) || (changeLon == true)) {
				...
				if (AntennaDataCollect) {
					...
					var dist = google.maps.geometry.spherical.computeDistanceBetween(siteLatLon, endLatLon);
					var bearing = google.maps.geometry.spherical.computeHeading(siteLatLon, endLatLon);
					...
					if (!AntennaData[bearing] || typeof AntennaData[bearing] === 'undefined' || (dist > AntennaData[bearing] && dist < maxDist)) {
